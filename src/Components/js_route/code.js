...
export var encrypt = function (choice, path_type, totalCols, plain_text) {
  ...
  if (totalRows != math.floor(totalRows)) {
    ...
    totalRows = Math.floor(len(plain_text) / totalCols);
  }

  var new_grid = [];
  var encryptedText = "";
  var parallelepipedRepresentation = ""; // Reiniciar siempre como cadena vacía

  if (choice == "1" || choice == "2" || choice == "3") {
    new_grid = fill_table_for_encrypt(
      plain_text,
      totalRows,
      totalCols,
      path_type
    );
    parallelepipedRepresentation = print_parallelepiped2(new_grid); // No concatenar
    encryptedText = read_cipher_text(new_grid, totalRows, totalCols, path_type);
  } else if (choice == "4") {
    parallelepipedRepresentation = print_parallelepiped(plain_text); // No concatenar
    encryptedText = top_to_bottom(
      plain_text,
      totalCols,
  ...
  if (choice == "3") {
    encryptedText = encryptedText.__getslice__(0, null, 1);
  }
